...
#### logging IndexErrors
    if(c==0):
        ...
        ##############################################
        results["total_loss_Instrument"]=pd.Series()
        results["total_loss_Market"]=0.0
#### data includes nans and infs
    if(c==1):
        ...
        parameters["pnlKey"]="pnlKey"
        ##############################################
        results["total_loss_Instrument"]=pd.Series({'a':3.0,'b':2.0})
        results["total_loss_Market"]=5.0
#### sample data
    if(c==2):
        ...
        parameters["pnlKey"]="pnlKey"
        ##############################################
        results["total_loss_Instrument"]=pd.Series({'a':0.0,'b':0.0,'c':0.0,'d':0.0,'e':1.05})
        results["total_loss_Market"]=0
    if(c==3):
        indexlist=  [pd.tslib.Timestamp('2017-01-03 09:30:30'),
        ...
        parameters["pnlKey"]="pnlKey"
        ##############################################
        results["total_loss_Instrument"]=pd.Series({'a':2.75,'b':0.25})
        results["total_loss_Market"]=5.79
