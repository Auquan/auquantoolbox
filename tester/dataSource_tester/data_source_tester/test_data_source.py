...
##############  creating CsvDataSource object to test derived functions
            ...
            for inst in parameters["instrumentIds"]:
                result_allInstrumentUpdates[inst].getBookData().equals(dataSet["dsdict_getAllInstrumentUpdates"][inst])
            csvdatasource._bookDataByInstrument=result_allInstrumentUpdates
            csvdatasource.padInstrumentUpdates()
            for inst in parameters["instrumentIds"]:
                assert csvdatasource._bookDataByInstrument[inst].getBookData().round(3).equals(dataSet["dsdict_padInstrumentUpdates"][inst])
            ...
            for inst in parameters["instrumentIds"]:
                if i==0:
                    ...
                    for ele in csvdatasource._bookDataByInstrument[inst].getBookData():
                        assert csvdatasource._bookDataByInstrument[inst].getBookData()[ele].round(3).equals(dataSet["dsdict_processAllInstrumentUpdates"][inst][ele])
                if i==1:
                    assert csvdatasource._bookDataByInstrument[inst].getBookData().equals(dataSet["dsdict_processAllInstrumentUpdates"][inst])
