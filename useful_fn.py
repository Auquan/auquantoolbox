...
def get_vwap(bid_vol, bid_price, ask_price, ask_vol):
    ...
    if volume > 0:
        price = (np.sum(bid_price * ask_vol) + np.sum(ask_price * bid_vol)
                 ) / (100 * volume)  # Calculated for a vol = 0.12353
    else:
        price = (np.sum(bid_price) + np.sum(ask_price)) / \
            (200 * len(bid_price))
    return price
...
def get_exp_date(trade_date, all_dates, p=False):
    ...
    exp_date = pd.to_datetime(trade_date).date() + \
