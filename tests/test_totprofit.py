...
def test_totalprofit(mock_instrumentmanager, mock_instrumentlookbackdata):
        ...
        for i in range(0,5):
                ...
                resultforInstrument=TotalProfitFeature.computeForInstrument(i, "", dataSet["featureParams"], "featureKey", mock_instrumentmanager)
                mock_instrumentmanager.getDataDf.return_value = dataSet["data"]
                resultforMarket = TotalProfitFeature.computeForMarket(i, "", dataSet["featureParams"], "featureKey", {}, mock_instrumentmanager)
                assert (resultforInstrument.tolist()) == dataSet["totpi"]
                assert [float(resultforMarket)] == dataSet["totpm"]
