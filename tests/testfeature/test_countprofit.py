...
def test_countprofit(mock_instrumentmanager, mock_instrumentlookbackdata):
        ...
        for i in range(0,5):
                ...
                resultforInstrument=CountProfitFeature.computeForInstrument(i, "", dataSet["featureParams"], "featureKey", mock_instrumentmanager)
                mock_instrumentmanager.getDataDf.return_value = dataSet["data"]
                resultforMarket = CountProfitFeature.computeForMarket(i, "", dataSet["featureParams"], "featureKey", {}, mock_instrumentmanager)
                assert (resultforInstrument.tolist()) == dataSet["cntpi"]
                assert [float(resultforMarket)] == dataSet["cntpm"]
